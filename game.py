...
class Game:
    ...
    def draw_instructions(self):
        ...
        for i, text in enumerate(instructions):
            instruction_text = font.render(text, True, BLACK)
            SCREEN.blit(instruction_text, (50, SCREEN.get_height() - 150 + i * 30))
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if self.question_manager.show_question and not self.question_manager.question_answered:
                            ...
                            result = self.dice.roll_dice_animation(self.board, self.current_player().position, draw_funcs)
                            if result:
